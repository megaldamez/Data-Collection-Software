...
for j=1:NumFrm
    ...
    for i = 1:16
        ...
        DataArray(:,i) =  Array(1:Len)';
    end
    meanDataArray(:,j) = mean(DataArray,2);
    %     flushinput(s)
    %     fgetl(s);
    %     %     angle = fgetl(s)
    %     DataAngle(j) = str2double(fgetl(s));
    %     DataArray(:,j) =  Array(1:Len)';
    DataArray_f(:,j) = convn(meanDataArray(:,j), Filt.BPF', 'same');
...
% for i = 1:7000
   line = mean(meanDataArray,2);
% end
for i = 1:size(meanDataArray,2)
DataArray2(:,i) = meanDataArray(:,i) - line;
end
env = abs(hilbert(DataArray2));
env2 = env/max(max(env(2000:end)));
imagesc(db(env2(1:end,:)),[-50 0])
colormap(gray)